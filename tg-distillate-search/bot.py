...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        catalog = get_meta(conn, "hrtz_catalog_count") or "0"
    ...
    text = (
        ...
        f"Внешние источники: сайт <b>{hrtz}</b> статей, каталог <b>{catalog}</b> товаров, "
        f"YouTube <b>{youtube}</b> видео (обновлено {_esc(sources_updated)})\n"
        f"Источники:\n" + "\n".join(src_lines) + "\n"
